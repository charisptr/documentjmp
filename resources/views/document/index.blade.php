                    ...
                    <div class="row" style="padding-bottom: 25px">
                        <div class="col-md-12">
                            <button type="button" class="btn btn-primary" data-toggle="modal"
                                data-target="#modal-add-doc">Tambah Dokumen Baru</button>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        ...
                        <table id="docs-table" class="table table-striped table-bordered" style="width: 100%">
                            <thead>
                                ...
                                <tr>
                                    <th class="text-center"><input id="filter-folder" type="text"
                                            class="form-control" placeholder="Filter Folder"></th>
                                    <th class="text-center"><input id="filter-name" type="text" class="form-control"
                                            placeholder="Filter Nama File"></th>
                            ...
                            <tbody>
                                ...
                                @foreach ($documents as $document)
                                    <tr>
                                        @if ($document->folder)
                                            <td class="text-center">{{ $document->folder->name }}</td>
                                        @else
                                        ...
                                        <td class="text-center">
                                            ...
                                            @elseif ((clone $nowTime)->modify('+1 month') >= $document->expired_at)
                                        ...
                                        <td class="text-center"><a
                                                href="{{ route('document.download', $document->id) }}">Download</a>
                                        </td>
                                        <td class="text-center">
                                            <button type="button" class="btn btn-sm btn-primary" data-toggle="modal"
                                                data-target="#modal-update-doc-{{ $document->id }}">Edit</button>
                                            <form method="POST" style="display:inline"
                                                action="{{ route('document.destroy', $document->id) }}">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
                                                    class="btn btn-sm btn-danger button-delete-document">Delete</button>
                                            </form>
                                        </td>
                                    </tr>
                                @endforeach
...
@section('moreJS')
    <script>
        ...
        $(document).ready(function() {
            ...
            $('#filter-folder').on('keyup', function() {
                docsTable
                    .columns(folderColIdx)
                    .search(this.value)
                    .draw();
            });
...
<div class="modal fade" id="modal-add-doc" tabindex="-1" aria-labelledby="modal-add-doc" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="{{ route('document.store') }}" method="post" enctype="multipart/form-data" lang="id">
                @csrf
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        <label for="add-doc-folder" class="col-form-label">Folder:</label>
                        <select class="form-select" id="add-doc-folder" name="folder_id">
                            <option selected value="">(Tanpa folder)</option>
                    ...
                    <div class="mb-3">
                        <label for="add-doc-filename" class="col-form-label">Nama File:</label>
                        <input type="text" class="form-control" id="add-doc-filename" name="name"
                            value="{{ old('name', '') }}" required>
                    </div>
                    <div class="mb-3">
                        <label for="add-doc-exp-date" class="col-form-label">Tanggal Expired:</label>
                        <input type="date" class="form-control" id="add-doc-exp-date" name="expired_at"
                            value="{{ old('expired_at', '') }}" required>
                    </div>
                    <div class="mb-3">
                        <label for="add-doc-file" class="col-form-label">Upload File:</label>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="{{ route('document.update', $document->id) }}" method="POST"
                    ...
                    <div class="modal-body">
                        <div class="mb-3">
                            <label for="update-doc-folder-{{ $document->id }}" class="col-form-label">Folder:</label>
                            <select class="form-select" id="update-doc-folder-{{ $document->id }}" name="folder_id">
                                <option selected value="">(Tanpa folder)</option>
                                @foreach ($folders as $folder)
                                    <option value="{{ $folder->id }}"
                                        {{ $document->folder_id == $folder->id ? 'selected' : '' }}>
                                        {{ $folder->name }}</option>
                                @endforeach
                            </select>
                        </div>
